"""Contributions of ST and GH joints to PoE, elevation, and axial rotation by axis
...
if __name__ == '__main__':
    ...
    config_dir = Path(mod_arg_parser('Contributions of ST and GH joints to PoE, elevation, and axial rotation by axis',
    ...
    for idx, (activity, activity_df) in enumerate(db_elev.groupby('Activity', observed=True)):
        ...
        # means and standard deviations
        st_elev_mean_max = np.rad2deg(np.mean(trajs_st_elev_max, axis=0))
        st_axial_mean_max = np.rad2deg(np.mean(trajs_st_axial_max, axis=0))
        st_poe_mean_max = np.rad2deg(np.mean(trajs_st_poe_max, axis=0))
        gh_elev_mean_max = np.rad2deg(np.mean(trajs_gh_elev_max, axis=0))
        gh_axial_mean_max = np.rad2deg(np.mean(trajs_gh_axial_max, axis=0))
        gh_poe_mean_max = np.rad2deg(np.mean(trajs_gh_poe_max, axis=0))

        st_elev_sd_max = np.rad2deg(np.std(trajs_st_elev_max, axis=0, ddof=1))
        st_axial_sd_max = np.rad2deg(np.std(trajs_st_axial_max, axis=0, ddof=1))
        st_poe_sd_max = np.rad2deg(np.std(trajs_st_poe_max, axis=0, ddof=1))
        gh_elev_sd_max = np.rad2deg(np.std(trajs_gh_elev_max, axis=0, ddof=1))
        gh_axial_sd_max = np.rad2deg(np.std(trajs_gh_axial_max, axis=0, ddof=1))
        gh_poe_sd_max = np.rad2deg(np.std(trajs_gh_poe_max, axis=0, ddof=1))

        # plot endpoints
        axs[0, cur_row].errorbar(max_pos - 3, st_elev_mean_max, yerr=st_elev_sd_max,
                                 color=color_map.colors[0], marker=markers[0], capsize=3)
        axs[0, cur_row].errorbar(max_pos, gh_elev_mean_max, yerr=gh_elev_sd_max,
                                 color=color_map.colors[1], marker=markers[0], capsize=3)
        axs[1, cur_row].errorbar(max_pos - 3, st_axial_mean_max, yerr=st_axial_sd_max,
                                 color=color_map.colors[0], marker=markers[1], capsize=3)
        axs[1, cur_row].errorbar(max_pos, gh_axial_mean_max, yerr=gh_axial_sd_max,
                                 color=color_map.colors[1], marker=markers[1], capsize=3)
        axs[2, cur_row].errorbar(max_pos - 3, st_poe_mean_max, yerr=st_poe_sd_max,
                                 color=color_map.colors[0], marker=markers[2], capsize=3)
        axs[2, cur_row].errorbar(max_pos, gh_poe_mean_max, yerr=gh_poe_sd_max,
                                 color=color_map.colors[1], marker=markers[2], capsize=3)
    ...
    axs[0, 0].set_title('CA', y=0.8)
    axs[0, 1].set_title('SA', y=0.8)
    axs[0, 2].set_title('FE', y=0.8)
