...
if __name__ == '__main__':
    ...
    config_dir = Path(mod_arg_parser('Compare HT, ST, GH axial rotation for elevation trials by gender',
    ...
    for act_idx, (activity, activity_df) in enumerate(db_elev.groupby('Activity', observed=True)):
        ...
        all_traj_st_f = all_traj_ht_f - all_traj_gh_f

        all_traj_ht_m = np.stack(m_df['traj_interp'].apply(
            extract_sub_rot_norm, args=['ht', 'common_fine_up', 'true_axial_rot', None, 'up']), axis=0)
        all_traj_gh_m = np.stack(m_df['traj_interp'].apply(
            extract_sub_rot_norm, args=['gh', 'common_fine_up', 'true_axial_rot', None, 'up']), axis=0)
        all_traj_st_m = all_traj_ht_m - all_traj_gh_m

        # means and standard deviations
        ht_mean_f = np.rad2deg(np.mean(all_traj_ht_f, axis=0))
        gh_mean_f = np.rad2deg(np.mean(all_traj_gh_f, axis=0))
        st_mean_f = np.rad2deg(np.mean(all_traj_st_f, axis=0))
        ht_sd_f = np.rad2deg(np.std(all_traj_ht_f, ddof=1, axis=0))
        gh_sd_f = np.rad2deg(np.std(all_traj_gh_f, ddof=1, axis=0))
        st_sd_f = np.rad2deg(np.std(all_traj_st_f, ddof=1, axis=0))

        ht_mean_m = np.rad2deg(np.mean(all_traj_ht_m, axis=0))
        gh_mean_m = np.rad2deg(np.mean(all_traj_gh_m, axis=0))
        st_mean_m = np.rad2deg(np.mean(all_traj_st_m, axis=0))
        ht_sd_m = np.rad2deg(np.std(all_traj_ht_m, ddof=1, axis=0))
        gh_sd_m = np.rad2deg(np.std(all_traj_gh_m, ddof=1, axis=0))
        st_sd_m = np.rad2deg(np.std(all_traj_st_m, ddof=1, axis=0))

        # spm
        ht_f_vs_m = spm_test(all_traj_ht_f, all_traj_ht_m).inference(alpha, two_tailed=True, **infer_params)
        st_f_vs_m = spm_test(all_traj_st_f, all_traj_st_m).inference(alpha, two_tailed=True, **infer_params)
        gh_f_vs_m = spm_test(all_traj_gh_f, all_traj_gh_m).inference(alpha, two_tailed=True, **infer_params)

        # plot mean and SD
        cur_row = act_row[activity.lower()]
        ht_ln_f = mean_sd_plot(axs[cur_row, 0], x, ht_mean_f, ht_sd_f,
                               dict(color=color_map.colors[0], alpha=0.25),
                               dict(color=color_map.colors[0], marker=markers[0], markevery=20))
        st_ln_f = mean_sd_plot(axs[cur_row, 1], x, st_mean_f, st_sd_f,
                               dict(color=color_map.colors[0], alpha=0.25),
                               dict(color=color_map.colors[0], marker=markers[1], markevery=20))
        gh_ln_f = mean_sd_plot(axs[cur_row, 2], x, gh_mean_f, gh_sd_f,
                               dict(color=color_map.colors[0], alpha=0.25),
                               dict(color=color_map.colors[0], marker=markers[2], markevery=20))

        ht_ln_m = mean_sd_plot(axs[cur_row, 0], x, ht_mean_m, ht_sd_m,
                               dict(color=color_map.colors[1], alpha=0.25),
                               dict(color=color_map.colors[1], marker=markers[0], markevery=20))
        st_ln_m = mean_sd_plot(axs[cur_row, 1], x, st_mean_m, st_sd_m,
                               dict(color=color_map.colors[1], alpha=0.25),
                               dict(color=color_map.colors[1], marker=markers[1], markevery=20))
        gh_ln_m = mean_sd_plot(axs[cur_row, 2], x, gh_mean_m, gh_sd_m,
                               dict(color=color_map.colors[1], alpha=0.25),
        ...
        if act_idx == 0:
            leg_mean.append(ht_ln_f[0])
            leg_mean.append(st_ln_f[0])
            leg_mean.append(gh_ln_f[0])
            leg_mean.append(ht_ln_m[0])
            leg_mean.append(st_ln_m[0])
            leg_mean.append(gh_ln_m[0])
    ...
    leg_right = fig.legend(leg_mean, ['HT F', 'ST F', 'GH F', 'HT M', 'ST M', 'GH M'], loc='upper right',
