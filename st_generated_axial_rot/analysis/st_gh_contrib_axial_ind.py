"""Compare contributions of the ST and GH joint to axial rotation on a per-trial basis
...
if __name__ == '__main__':
    ...
    config_dir = Path(mod_arg_parser('Compare contributions of the ST and GH joint to axial rotation on a per-trial '
...
#%%
    ...
    for activity, activity_df in db_elev.groupby('Activity', observed=True):
        # overall
        pdf_file_path = output_path / ('ind_axial_' + activity + '_' + params.torso_def +
        ...
        with PdfPages(pdf_file_path) as activity_pdf:
            for trial_name, df_row in activity_df.iterrows():
                ...
                ax.plot(ht_elev, np.rad2deg(st_traj.contribs[sec, 2] + gh_traj.contribs[sec, 2]), label='Axial Sum',
                        color='k')

                ax.plot(ht_elev, np.rad2deg(np.abs(st_traj.contribs[sec, 2]) - np.abs(gh_traj.contribs[sec, 2])), '--', label='Axial Diff',
                        color='k')
