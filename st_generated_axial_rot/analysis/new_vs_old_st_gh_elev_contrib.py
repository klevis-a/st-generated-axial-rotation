...
#%%
    ...
    for idx, (activity, activity_df) in enumerate(db_elev.groupby('Activity', observed=True)):
        ...
        axs[cur_row, 0].plot(st_x_sig, np.repeat(0, st_x_sig.size), color='k', lw=2)
    ...
    fig.suptitle('ST and GH Elevation Contribution Comparison', x=0.47, y=0.99, fontweight='bold')
    plt.subplots_adjust(top=0.93)
    leg_left = fig.legend(leg_mean[:2], ['ST Euler', 'ST Contrib'], loc='upper left', bbox_to_anchor=(0, 1),
                          ncol=1, handlelength=1.5, handletextpad=0.5, columnspacing=0.75, labelspacing=0.3,
                          borderpad=0.2)
    leg_right = fig.legend(leg_mean[2:], ['GH Euler', 'GH Contrib'], loc='upper right', bbox_to_anchor=(1, 1),
                           ncol=1, handlelength=1.5, handletextpad=0.5, columnspacing=0.75, labelspacing=0.3,
                           borderpad=0.2)
