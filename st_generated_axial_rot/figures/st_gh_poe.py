"""ST and GH Contributions to HT PoE
...
if __name__ == '__main__':
    ...
    config_dir = Path(mod_arg_parser('ST and GH Contributions to HT PoE',
    ...
    for i in range(3):
        style_axes(axs_elev[i], 'Humerothoracic Elevation (Deg)' if i == 2 else None, 'PoE Contribution (Deg)')

    for i in range(2):
        style_axes(axs_er[i], 'Motion Completion (%)' if i == 1 else None, 'PoE Contribution (Deg)')
    ...
    for idx_act, (activity, activity_df) in enumerate(db_er_endpts.groupby('Activity', observed=True)):
        traj_st = np.stack(activity_df['st_contribs_interp'], axis=0)[:, :, 0]
        traj_gh = np.stack(activity_df['gh_contribs_interp'], axis=0)[:, :, 0]
    ...
    fig.suptitle('ST and GH Contributions to HT PoE', x=0.5, y=0.99, fontweight='bold')
