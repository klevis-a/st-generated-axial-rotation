"""Plot HT, ST, and GH contribution to axial rotation and compare ST and GH contributions against HT contributions
...
if __name__ == '__main__':
    ...
    from st_generated_axial_rot.common.plot_utils import (init_graphing, make_interactive, mean_sd_plot, style_axes,
                                                          ...
                                                          sig_filter, extract_sig, non_sig_filter)
    ...
    config_dir = Path(mod_arg_parser('Plot HT, ST, and GH contribution to axial rotation and compare ST and GH '
                                     'contributions against HT contributions', __package__, __file__))
    ...
    for idx, (activity, activity_df) in enumerate(db_elev.groupby('Activity', observed=True)):
        ...
        ht_vs_gh = spm_test(traj_ht - traj_gh, 0).inference(alpha, two_tailed=True, **infer_params)
        ht_vs_st = spm_test(traj_ht - traj_st, 0).inference(alpha, two_tailed=True, **infer_params)
        x_gh_sig = non_sig_filter(ht_vs_gh, x_elev)
        x_st_sig = non_sig_filter(ht_vs_st, x_elev)
        axs_elev[cur_row].plot(x_gh_sig, np.repeat(spm_y[activity.lower()], x_gh_sig.size), color=color_map.colors[0])
        axs_elev[cur_row].plot(x_st_sig, np.repeat(spm_y[activity.lower()], x_st_sig.size), color=color_map.colors[1])

        print('Activity: {}'.format(activity))
        print('GH different than HT')
        print(extract_sig(ht_vs_gh, x_elev))
        print('P-values: ')
        print(output_spm_p(ht_vs_gh))
        print('ST different than HT')
        print(extract_sig(ht_vs_st, x_elev))
        print('P-values: ')
        print(output_spm_p(ht_vs_st))
    ...
    fig.suptitle('GH and ST Contributions to HT Axial Rotation', x=0.5, y=0.99, fontweight='bold')
