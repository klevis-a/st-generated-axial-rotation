"""ST contributions to HT axial rotation by age group
...
if __name__ == '__main__':
    ...
    config_dir = Path(mod_arg_parser('ST contributions to HT axial rotation by age group',
    ...
    for idx, (activity, activity_df) in enumerate(db_elev.groupby('Activity', observed=True)):
        ...
        traj_st_gt45 = np.stack(gt45_df['traj_interp'].apply(
            extract_sub_rot_norm, args=['st', 'common_fine_up', 'contribs', 2, 'up']), axis=0)

        traj_st_lt35_max = np.stack(lt35_df['traj_interp'].apply(
            sub_rot_at_max_elev, args=['st', 'contribs', 2, 'up']), axis=0)
        traj_st_gt45_max = np.stack(gt45_df['traj_interp'].apply(
            sub_rot_at_max_elev, args=['st', 'contribs', 2, 'up']), axis=0)
        ...
        st_lt35_ln = mean_sd_plot(axs_elev[cur_row], x_elev, st_lt35_mean, st_lt35_sd,
                                  dict(color=color_map.colors[0], alpha=0.25),
                                  dict(color=color_map.colors[0], marker=markers[0], markevery=20))
        st_gt45_ln = mean_sd_plot(axs_elev[cur_row], x_elev, st_gt45_mean, st_gt45_sd,
                                  dict(color=color_map.colors[1], alpha=0.25),
                                  dict(color=color_map.colors[1], marker=markers[1], markevery=20))

        axs_elev[cur_row].errorbar(max_pos, st_lt35_max_mean, yerr=st_lt35_max_sd, color=color_map.colors[0],
                                   marker=markers[0], capsize=3)
        axs_elev[cur_row].errorbar(max_pos, st_gt45_max_mean, yerr=st_gt45_max_sd, color=color_map.colors[1],
                                   marker=markers[1], capsize=3)

        # spm
        lt35_vs_gt45 = spm_test(traj_st_lt35, traj_st_gt45).inference(alpha, two_tailed=True, **infer_params)
    ...
    for idx_act, (activity, activity_df) in enumerate(db_er_endpts.groupby('Activity', observed=True)):
        ...
        st_lt35_ln = mean_sd_plot(axs_er[idx_act], x_er, st_lt35_mean, st_lt35_sd,
                                  dict(color=color_map.colors[0], alpha=0.25),
                                  dict(color=color_map.colors[0], marker=markers[0], markevery=20))
        st_gt45_ln = mean_sd_plot(axs_er[idx_act], x_er, st_gt45_mean, st_gt45_sd,
                                  dict(color=color_map.colors[1], alpha=0.25),
                                  dict(color=color_map.colors[1], marker=markers[1], markevery=20))

        lt35_vs_gt45 = spm_test(traj_st_lt35[:, 1:], traj_st_gt45[:, 1:]).inference(alpha, two_tailed=True,
                                                                                    **infer_params)
        x_sig = sig_filter(lt35_vs_gt45, x_er[1:])
        axs_er[idx_act].plot(x_sig, np.repeat(2, x_sig.size), color='k')
        print(activity)
        print(extract_sig(lt35_vs_gt45, x_er[1:]))

    # figure title and legend
    plt.tight_layout(pad=0.25, h_pad=1.5, w_pad=0.5)
    fig.suptitle('ST Contributions to HT Axial Rotation by Age Group', x=0.5, y=0.99, fontweight='bold')
    plt.subplots_adjust(top=0.93)
    leg_left = fig.legend(leg_elev_mean, ['<35', '>45'], loc='upper left', bbox_to_anchor=(0, 1),
